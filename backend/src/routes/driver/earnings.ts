  ...
  async (req: Request, res: Response) => {
    try {
      ...
      completedRides.forEach((ride) => {
        const seatsBooked = ride.bookings.reduce(
          (sum, booking) => sum + (booking.numberOfSeats || 1),
          0
        );
        ...
        if (ride.bookings.length === 0) {
          console.warn(
            `[Earnings] Ride ${ride.id} has no bookings - earnings will be $0`
          );
        }
        ...
        if (rideEarnings === 0 && grossRideEarnings > 0) {
          console.log(
            `[Earnings] Ride ${ride.id}: Gross=${grossRideEarnings}, Net=${rideEarnings} (fees charged to riders)`
          );
        }
        ...
        const displayDate = new Date(ride.updatedAt).toLocaleDateString(
          "en-US",
          {
            month: "short",
            day: "numeric",
        ...
        earningsByDate[displayDate] =
          (earningsByDate[displayDate] || 0) + rideEarnings;
        ...
        rideDetails.push({
          ...
          earningsBreakdown: {
            grossEarnings: parseFloat(
              earningsBreakdown.grossEarnings.toFixed(2)
            ),
