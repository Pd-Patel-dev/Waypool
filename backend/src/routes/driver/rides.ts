...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (isRecurring === true) {
        if (
          ...
          return sendBadRequest(
            res,
            'Recurring pattern must be "daily", "weekly", or "monthly" when isRecurring is true'
          );
        }
      }
      ...
      if (!isDraft) {
        ...
        try {
          ...
          for (const existingRide of existingRides) {
            const existingDateTime = parseDateTime(
              existingRide.departureDate,
              existingRide.departureTime
            );
            const timeDifference = Math.abs(
              newRideDateTime.getTime() - existingDateTime.getTime()
            );
      ...
      const ride = await prisma.rides.create({
        data: {
          ...
          ...(estimatedTimeMinutes !== undefined && {
            estimatedTimeMinutes: parseInt(estimatedTimeMinutes),
          }),
      ...
      const departureISO = parseDateTimeToISO(
        ride.departureDate,
        ride.departureTime
      );

      // Send email notifications to riders whose last dropoff location is within 30 miles of ride starting point (only for non-draft rides)
      if (!isDraft) {
        // Execute email sending asynchronously without blocking the response
        (async () => {
          try {
            console.log(`📧 [Ride ${ride.id}] Starting ride notification process from ${ride.fromCity} to ${ride.toCity}`);
            console.log(`📧 [Ride ${ride.id}] Ride coordinates: ${ride.fromLatitude}, ${ride.fromLongitude}`);
            
            // Find all riders and their last dropoff location from their most recent completed booking
            const allRiders = await prisma.users.findMany({
              where: {
                isRider: true,
              },
              select: {
            ...
            console.log(`📧 [Ride ${ride.id}] Found ${allRiders.length} riders`);

            // Filter riders whose last dropoff location is within 30 miles of the ride's starting point
            const RIDE_NOTIFICATION_RADIUS_MILES = 30;
            const nearbyRiders = allRiders.filter((rider) => {
              // Get the last dropoff location from the most recent completed booking
              const lastBooking = rider.bookings[0];
              if (!lastBooking || !lastBooking.rides) {
                return false;
              }

              const dropoffLatitude = lastBooking.rides.toLatitude;
              const dropoffLongitude = lastBooking.rides.toLongitude;

              if (!dropoffLatitude || !dropoffLongitude) {
                return false;
              }

              // Calculate distance from ride's starting location to rider's last dropoff location
              ...
              const isNearby = distance <= RIDE_NOTIFICATION_RADIUS_MILES;
              if (isNearby) {
                console.log(`📧 [Ride ${ride.id}] Rider ${rider.fullName} (${rider.email}) last dropoff is ${distance.toFixed(2)} miles from ride start - within notification radius`);
              }
            ...
            console.log(`📧 [Ride ${ride.id}] Found ${nearbyRiders.length} riders with last dropoff within ${RIDE_NOTIFICATION_RADIUS_MILES} miles of ride starting point`);

            // Send emails to nearby riders
            if (nearbyRiders.length > 0) {
              try {
                ...
                if (!hasGmailConfig) {
                  console.error(`❌ [Ride ${ride.id}] Gmail API credentials not configured. Cannot send emails.`);
                  console.error(`❌ [Ride ${ride.id}] Required env vars: GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET, GMAIL_REFRESH_TOKEN, GMAIL_SENDER_EMAIL`);
                  return; // Exit early if credentials are missing
                }

                const { sendRideNotificationEmail } = await import(
                  "../../services/emailService"
                );

                // Get driver info for email (use driverName from request body, fallback to database)
                const driverInfo = await prisma.users.findUnique({
                  where: { id: driverId },
                  select: {
                    fullName: true,
                  },
                ...
                const driverNameForEmail =
                  driverName || driverInfo?.fullName || "A driver";

                console.log(
                  `📧 [Ride ${ride.id}] Sending ride notification emails to ${nearbyRiders.length} nearby rider(s). Driver: ${driverNameForEmail}`
                );
                ...
                const emailResults = await Promise.allSettled(
                  nearbyRiders.map(async (rider) => {
                    if (!rider.email) {
                      console.warn(`⚠️  [Ride ${ride.id}] Rider ${rider.id} (${rider.fullName}) has no email address`);
                      return;
                    }

                    try {
                      console.log(`📧 [Ride ${ride.id}] Attempting to send email to ${rider.email}...`);
                      await sendRideNotificationEmail({
                        riderEmail: rider.email,
                        riderName: rider.fullName || 'Rider',
                        driverName: driverNameForEmail,
                        rideDetails: {
                      ...
                      console.log(`✅ [Ride ${ride.id}] Successfully sent ride notification email to ${rider.email}`);
                    } catch (emailError: any) {
                      const errorMessage = emailError?.message || String(emailError);
                      const errorStack = emailError?.stack || 'No stack trace';
                      console.error(
                        `❌ [Ride ${ride.id}] Failed to send ride notification email to ${rider.email}:`,
                        errorMessage
                      );
                      console.error(`❌ [Ride ${ride.id}] Error details:`, errorStack);
                ...
                console.log(`📧 [Ride ${ride.id}] Email sending complete: ${successful} successful, ${failed} failed out of ${nearbyRiders.length} total`);
                ...
                if (failed > 0) {
                  emailResults.forEach((result, index) => {
                    if (result.status === 'rejected') {
                      ...
                      console.error(`❌ [Ride ${ride.id}] Failed email ${index + 1} (${nearbyRiders[index]?.email}):`, errorMessage);
                      if (error?.stack) {
                        console.error(`❌ [Ride ${ride.id}] Stack trace:`, error.stack);
                      }
                    }
                  });
                }
                ...
                console.error(
                  `❌ [Ride ${ride.id}] Error importing or using email service:`,
                  emailServiceError?.message || String(emailServiceError)
                );
                console.error(`❌ [Ride ${ride.id}] Stack trace:`, emailServiceError?.stack);
              }
            } else {
              console.log(
                `📧 [Ride ${ride.id}] No riders found within ${RIDE_NOTIFICATION_RADIUS_MILES} miles of ride starting point (${ride.fromAddress})`
              );
            }
          } catch (notificationError) {
            console.error(
              `❌ [Ride ${ride.id}] Error processing ride notifications:`,
              notificationError
            );
            // Don't throw - notification failures shouldn't break ride creation
          }
        })(); // Immediately invoke async function
      } else {
        console.log(`📧 [Ride Creation] Skipping email notifications - ride is a draft`);
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const formattedRides = rides.map((ride) => {
        ...
        const earningsBreakdown = calculateRideEarnings(
          ride.pricePerSeat || 0,
          ride.bookings
        );
...
router.delete("/:id", async (req: Request, res: Response) => {
  try {
    ...
    const rideId = parseInt(rideIdParam);
    const driverId =
      req.query.driverId && typeof req.query.driverId === "string"
    ...
    try {
      ...
      if (ride.status === "in-progress") {
        return res.status(400).json({
          success: false,
          message:
            "Cannot edit a ride that is in progress. Please complete or cancel the ride first.",
        });
      }

      // Extract update fields from request body
      const { departureDate, departureTime, pricePerSeat, availableSeats } =
        req.body;
      ...
      if (bookingsCount > 0 && changes.length > 0) {
        ...
        const { sendRideModificationEmail } = await import("../../services/emailService");

        const notificationPromises = bookings.map(
          async (booking: (typeof bookings)[0]) => {
            ...
            const notificationPromise = prisma.notifications
              ...
              .catch((error) => {
                console.error(
                  `❌ Error creating notification for rider ${booking.riderId}:`,
                  error
                );
                return null;
              });
            ...
            const emailPromise = sendRideModificationEmail({
...
router.put("/:id/cancel", async (req: Request, res: Response) => {
  try {
    ...
    // Send cancellation emails to all riders with confirmed bookings
    if (ride.bookings.length > 0) {
      const { sendRideCancellationEmail } = await import("../../services/emailService");

      const emailPromises = ride.bookings.map((booking) =>
        sendRideCancellationEmail({
          ...
          console.error(
            `❌ Error sending cancellation email to rider ${booking.riderId}:`,
            error
          );
        })
      );
    ...
    try {
      ...
      if (!ride.bookings || ride.bookings.length === 0) {
        return res.status(400).json({
          success: false,
          message:
            "Cannot start ride. At least one confirmed booking is required.",
        });
      }
...
router.put("/:id/complete", async (req: Request, res: Response) => {
  try {
    ...
    if (isTestMode) {
      console.log(
        `🧪 TEST MODE DETECTED: ENABLE_TEST_MODE=true is set. Bypassing all location validations.`
      );
    }
    ...
    if (!isTestMode) {
      ...
      if (isNaN(distanceToDestination) || !isFinite(distanceToDestination)) {
        return res.status(400).json({
          success: false,
          message:
            "Invalid location coordinates. Please ensure location services are enabled and try again.",
      ...
      if (distanceToDestination > MAX_DISTANCE_METERS) {
        ...
        return res.status(400).json({
          success: false,
          message: `You must be within ${MAX_DISTANCE_METERS} meters (${maxDistanceInFeet} feet) of the destination to complete the ride. You are currently ${Math.round(
            distanceToDestination
          )} meters (${distanceInFeet} feet) away.`,
      ...
      console.log(
        `🧪 TEST MODE IS ON: Destination verification bypassed. Driver ${driverId} can complete the ride regardless of location.`
      );
    }
    ...
    if (ride.bookings.length > 0) {
      ...
      if (!allPickedUp) {
        ...
        return res.status(400).json({
          success: false,
          message: `Cannot complete ride. ${unpickedCount} passenger${
            unpickedCount !== 1 ? "s" : ""
          } still need${
            unpickedCount !== 1 ? "" : "s"
          } to be marked as picked up.`,
        });
      }
    }
    ...
    if (ride.bookings.length > 0) {
      ...
      console.warn(
        `[CompleteRide] Ride ${rideId} has no bookings - totalEarnings will be $0`
      );
    }
          ...
          try {
            ...
            const errorMessage =
              captureError instanceof Error
                ...
                : "Unknown error";
            console.error(
              `❌ Error capturing payment for booking ${booking.id}:`,
              captureError
            );
    ...
    const passengers =
      (completedRide as CompletedRideWithBookings | null)?.bookings.map(
        (booking) => ({
          id: booking.id,
          riderId: booking.riderId,
          riderName: booking.users?.fullName,
    ...
    const departureISO = completedRide
      ? parseDateTimeToISO(
          completedRide.departureDate,
          completedRide.departureTime
        )
      : "";
