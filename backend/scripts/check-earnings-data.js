...
async function checkEarningsData() {
    try {
        ...
        const completedRides = await prisma_1.prisma.rides.findMany({
            where: {
                status: 'completed',
            },
            include: {
        ...
        if (completedRides.length === 0) {
            ...
            completedRides.forEach((ride, index) => {
                ...
                const totalPassengers = ride.bookings.reduce((sum, b) => sum + (b.numberOfSeats || 1), 0);
...
checkEarningsData();
//# sourceMappingURL=check-earnings-data.js.map